...
export class leaveapplycomponent implements OnInit {
    ...
    ngOnInit(): void {
        this.leavemodi_id = this.route.snapshot.paramMap.get('leavemodi_id')
        this.leave_id = this.leavemodi_id
            ...
            if (to_value.length!=0){
                if(leave_value!='Select'){
                    ...
                    if(this.leave_id){
                        ...
                        .subscribe(
                            (response) => {
                                this.router.navigate(["/remotestate/leave"]);
                            }
                        )
                    }else{
                        this.githubService.postleaveapi(basebody,this.header)
                        .subscribe(
                            (response) => {
